...
def importingargs():
    ...
    parser.add_argument(
        "--mergeshellfilepath", help="This is the filepath of mergefile by shell script")

    parser.add_argument(
        "--outputfilepath", help="This is the filepath of outputfilepath")
    args = parser.parse_args()
    assert os.path.exists(
        args.col1filepath), "%s is not found" % args.col1filepath
    assert os.path.exists(
        args.col2filepath), "%s is not found" % args.col2filepath
    assert os.path.exists(
        args.mergeshellfilepath), "%s is not found" % args.mergefilepath
...
def mergecol1andcol2usingtab(col1filepath, col2filepath):
    ...
    mergelist = ["%s\t%s\n" % (linecol1.strip(), linecol2.strip(
    )) for linecol1, linecol2 in
        zip(open(col1filepath, "r"), open(col2filepath, "r"))]
...
def output(mergelist, mergeshellfilepath, outputfilepath):
    ...
    mergelistshell = ["%s\n" % line.strip()
                      for line in open(mergeshellfilepath, "r")]
