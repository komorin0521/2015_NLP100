...
def importingargs():
    ...
    parser.add_argument(
        "--resultpythondir", help="This is the output directory")
    ...
    assert os.path.exists(
        args.resultpythondir), "%s is not exists" % args.resultpythondir
...
def split(inputfilepath, splitnum):
    ...
    resultlist = list()
    resultlist = [sorted(inputlist[i * splitnum:(i + 1) * splitnum])
...
def output(resultshelldir, resultpythondir, resultlist):
    ...
    assert sorted(resultlist) == sorted(
        resultshelllist), "This program is not correct"
